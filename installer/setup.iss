; Inno Setup script for Creo Automation (cleaned / deduped)
[Setup]
...
begin
  ...
  // Service account for the Windows service (optional)
  var LabelSvc: TLabel;
  var EditSvc: TEdit;
  var HintSvc: TNewStaticText;
  LabelSvc := TLabel.Create(DBPage);
  LabelSvc.Parent := DBPage.Surface;
  LabelSvc.Left := 0; LabelSvc.Top := y; LabelSvc.Caption := 'Service account (optional):';
  EditSvc := TEdit.Create(DBPage);
  EditSvc.Parent := DBPage.Surface;
  EditSvc.Left := 0; EditSvc.Top := y + 18; EditSvc.Width := DBPage.SurfaceWidth;
  EditSvc.Hint := 'Optional service account name, e.g. DOMAIN\svcUser'; EditSvc.ShowHint := True;
  HintSvc := TNewStaticText.Create(DBPage);
  HintSvc.Parent := DBPage.Surface; HintSvc.Left := 4; HintSvc.Top := EditSvc.Top + 24; HintSvc.Width := DBPage.SurfaceWidth; HintSvc.Caption := 'If provided, ACLs on the encrypted password will allow this account to read it.';
  y := HintSvc.Top + 26;
  
  // Test Connection button
  TestBtn := TButton.Create(DBPage);
  TestBtn.Parent := DBPage.Surface;
  TestBtn.Left := DBPage.SurfaceWidth - 140; TestBtn.Top := y; TestBtn.Width := 130;
...
begin
  ...
  // Build a PowerShell command that invokes the bundled Node checker so we get an authenticated SSL test
