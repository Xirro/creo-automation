(function breakerDragDrop()
{
    ...
    if(_saveSectionBtn) {
        ...
        for(var j = 1; j < secNum + 1; j++){
            var name = 'section' + j;
            var list = document.getElementById(name).children;
            if (list.length == 0) {
                secMap.set(name, '');
            } else {
                for(var i = 0; i < list.length; i++) {
                    ...
                    if(secMap.has(name)) {
                        var temp = secMap.get(name);
                        temp += ',' + current;
                        secMap.set(name, temp);
                    }
                    else secMap.set(name, current);
                }
            }
        }
        if(secMap.size == 0)
            secMap.set('section 0', '0');

        function sectionFormSubmit(secMap) {
            var mbomID = document.getElementById('mbomID').value;
            var form = document.createElement('form');
            form.method = "POST";
            form.action = "../sectionConfigure";
            for (var el of secMap.keys()) {
                ...
                element2.value = breakers;
                element2.name = "idDev";
                form.appendChild(element2);

                element3.value = items;
                element3.name = "itemSumID";
                form.appendChild(element3);

                element4.value = mbomID;
                element4.name = "mbomID";
                form.appendChild(element4);
            }
            var element5 = document.createElement('input');
            element5.value = targets.length;
            element5.name = "totalSection";
