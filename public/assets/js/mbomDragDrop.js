(function breakerDragDrop()
{
    ...
    document.getElementById('saveSectionBtn').addEventListener('click', function() {
        let secArr = [];
        let secNum = (targets.length) - 3;
        for(let j = 1; j < secNum + 1; j++){
            let name = 'section' + j;
            let list = document.getElementById(name).children;
            for(let el of list) {
                secArr.push({
                    secNum: j,
                    ID: el.id
